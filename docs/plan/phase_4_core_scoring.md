...
The cross-encoder (`bge-reranker-v2-m3`) runs **entirely offline** in `preprocess.py`. It scores the **entire 5,000** candidate pool generated by Phase 1d RRF against the JD, min-max normalizes raw reranker logits to the same 0-100 scale as `core_score`, and saves both `ce_raw_score` and normalized `ce_score` to `artifacts/cross_encoder_scores.parquet`. At rank time, `rank.py` dynamically selects the Top 500 based on `core_score`, joins this parquet on `candidate_id` using Polars, and defensively normalizes legacy artifacts that may still contain raw logits.

**Scale invariant:** never blend raw cross-encoder logits directly with `core_score`. `core_score` is 0-100; `bge-reranker-v2-m3` raw outputs are typically small logits such as `-8..3`. The Phase 4 blend must use normalized `ce_score`, not `ce_raw_score`.
...
raw_scores = np.asarray(reranker.compute_score(pairs), dtype=np.float32)
if raw_scores.max() == raw_scores.min():
    ce_scores = np.full_like(raw_scores, 50.0)
else:
    ce_scores = (raw_scores - raw_scores.min()) / (raw_scores.max() - raw_scores.min()) * 100.0
...
ce_df = normalize_ce_scores(pl.read_parquet("artifacts/cross_encoder_scores.parquet"))
