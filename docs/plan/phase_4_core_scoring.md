...
The cross-encoder (`bge-reranker-v2-m3`) runs **entirely offline** in `preprocess.py`. It scores the configured top retrieval pool (`constants.CE_PRECOMPUTE_TOPK`) generated by Phase 1d against the JD, normalizes outputs to a 100.0 scale, and saves them to `artifacts/cross_encoder_scores.parquet`. At rank time, `rank.py` joins this parquet before the configurable Phase 5 candidate-pool slice, blends CE with `core_score`, and falls back to `core_score` when a widened-pool candidate has no CE score.

**Why not require CE for every widened-pool candidate?** The high-recall branch deliberately increases the feature pool first. CE can be expanded later, but missing CE scores should not block the recall experiment; fallback-to-core keeps ranking reproducible while we measure whether rescued candidates improve the top 100.
